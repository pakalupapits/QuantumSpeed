...
/* ---------------------------------------------------------
   3b. GLOBAL MISSION COUNTER
   Uses a free, no-signup counting API. This is a small
   community-run service, not guaranteed infrastructure —
   if it's ever unreachable we just hide the counter instead
   of breaking the page.
--------------------------------------------------------- */
const COUNTER_KEY = 'launchspeed_missions_launched_v1';
const COUNTER_BASE = 'https://countapi.mileshilliard.com/api/v1';

function renderCounterValue(n){
  const el = document.getElementById('mission-counter');
  if (n === null){
    el.style.display = 'none';
    return;
  }
  el.innerHTML = `<span class="counter-dot"></span> <b>${n.toLocaleString()}</b>&nbsp;missions launched worldwide`;
}

async function loadMissionCounter(){
  try {
    const res = await fetch(`${COUNTER_BASE}/get/${COUNTER_KEY}`, { cache: 'no-store' });
    if (!res.ok) throw new Error('no counter yet');
    const data = await res.json();
    renderCounterValue(Number(data.value) || 0);
  } catch (e) {
...
async function bumpMissionCounter(){
  try {
    const res = await fetch(`${COUNTER_BASE}/hit/${COUNTER_KEY}`, { cache: 'no-store' });
    const data = await res.json();
    renderCounterValue(Number(data.value) || 0);
  } catch (e) { /* service unreachable — silently skip */ }
}
...
/* ---------------------------------------------------------
   7b. EMAIL MY RESULTS
   No backend can send mail from a static site, so this opens
   the visitor's own email app with the results pre-filled —
   they hit send themselves. Zero setup, always works.
--------------------------------------------------------- */
let lastResult = null;

function buildResultsEmail(){
  if (!lastResult) return;
  const { ping, jitter, dl, ul, rank } = lastResult;
  const subject = 'My LAUNCHSPEED Mission Results';
  const body =
`MISSION REPORT — ${new Date().toLocaleString()}
...
document.getElementById('email-btn').addEventListener('click', buildResultsEmail);
...
btn.addEventListener('click', async () => {
  ...
  lastResult = { ping, jitter, dl, ul, rank };
